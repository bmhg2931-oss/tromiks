...
vi.mock("@/app/(app)/donations/stripe-sync-actions", () => ({
  fetchAndStageStripeHistoryPage: (...args: unknown[]) => mockFetchAndStage(...args),
}));

import { GET } from "@/app/api/cron/stripe-history-sync/route";

function makeRequest(authHeader?: string) {
  const headers: Record<string, string> = {};
  if (authHeader) headers.authorization = authHeader;
  return new NextRequest("http://localhost/api/cron/stripe-history-sync", { headers });
}
...
beforeEach(() => {
  ...
  mockFetchAndStage.mockResolvedValue({ ok: true, fetched: 0, staged: 0, skippedDuplicates: 0, reachedEnd: true, lastId: null });
});
...
describe("stripe-history-sync cron auth (fail-closed)", () => {
