...
export type DonationPaymentLineRow = {
  id: string;
  amount: number;
...
// שורות שיק/העברה מרובות שנשמרו בזרימת ההזנה המרובה (MultiPaymentLines) - מוצגות לקריאה
// בלבד בעת פתיחת רשומת התרומה לעריכה (עצמן לא ניתנות לעריכה כאן, רק תצוגה)
export async function getDonationPaymentLines(donationId: string): Promise<{ ok: boolean; lines?: DonationPaymentLineRow[]; error?: string }> {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from("donation_payment_lines")
    .select("id, amount, bank_name, branch_number, account_number, check_number, check_date, file_storage_path")
...
async function computeSurplus(
  ...
  const [{ data: pledges }, { data: donations }] = await Promise.all([
    supabase.from("pledges").select("amount, currency").eq("contact_id", contactId).neq("status", "בוטל").is("deleted_at", null),
    supabase.from("donations").select("amount, currency").eq("contact_id", contactId).is("deleted_at", null),
  ]);
  ...
  try {
    ...
    const rows = lines
      ...
      .map((l) => ({
        ...
        file_storage_path: l.filePath || null,
      }));
    if (rows.length > 0) {
...
export async function createDonation(
  ...
  return createDonationWithClient(supabase, user?.id ?? null, formData);
}

// גרסה שמקבלת לקוח Supabase מוזרק - נחוצה כדי שה-webhook של אישור סליקת אשראי (שאין
// לו session/cookies של משתמש מחובר) יוכל ליצור את התרומה בפועל דרך לקוח service-role,
// תוך שימוש באותה לוגיקה בדיוק כמו הזנה ידנית רגילה
export async function createDonationWithClient(
  supabase: SupabaseClient,
  userId: string | null,
  formData: FormData
): Promise<DonationFormResult> {
...
// יצירת זיכוי ידני ללקוח - נרשם כרשומת donations רגילה (payment_method='זיכוי'),
// כך שהוא מקטין את היתרה הפתוחה בדיוק כמו תשלום רגיל, אך מוצג ומסונן כ"זיכוי"
// ולא כ"תשלום" בהיסטוריה/בפעילות (ר' activityLine ב-ContactActivityTab/ContactHistoryTab)
export async function createCreditDonation(
  _prevState: DonationFormResult | null,
  formData: FormData
): Promise<DonationFormResult> {
  const supabase = await createClient();
  ...
  const contact_id = String(formData.get("contact_id") || "");
  if (!contact_id) return { ok: false, error: "חסר מזהה איש קשר" };

  const amount = Number(formData.get("amount") || 0);
  if (!amount || amount <= 0) return { ok: false, error: "יש להזין סכום זיכוי תקין" };
...
export async function updateDonation(
  ...
  // מסנכרן את שורות השיק/העברה המרובות: מוחק את הקיימות ומכניס מחדש את המצב הנוכחי
  // מהטופס - אותה שיטה בדיוק כמו ביצירה (insertPaymentLines), כדי שעריכה תשקף בדיוק
  // את מה שהוזן בפועל (כולל הוספה/הסרה של שורות)
  const { error: deleteLinesError } = await supabase.from("donation_payment_lines").delete().eq("donation_id", id);
  if (deleteLinesError) return { ok: false, error: deleteLinesError.message, errorCode: deleteLinesError.code };
  const linesError = await insertPaymentLines(supabase, id, formData);
  if (linesError) return { ok: false, error: linesError };
