...
export async function getContactBalances(supabase: SupabaseClient): Promise<Map<string, number>> {
  const [{ data: pledges }, { data: donations }] = await Promise.all([
    supabase.from("pledges").select("contact_id, amount, currency, pledge_date").neq("status", "בוטל").is("deleted_at", null),
    supabase.from("donations").select("contact_id, amount, currency, donation_date").is("deleted_at", null),
  ]);
...
export async function getContactTotalPledges(supabase: SupabaseClient): Promise<Map<string, number>> {
  const { data: pledges } = await supabase.from("pledges").select("contact_id, amount, currency, pledge_date").neq("status", "בוטל").is("deleted_at", null);
