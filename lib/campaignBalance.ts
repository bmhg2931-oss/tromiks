...
export async function getCampaignDirectTotals(supabase: SupabaseClient): Promise<Map<string, CampaignTotals>> {
  const [{ data: pledges }, { data: donations }] = await Promise.all([
    supabase.from("pledges").select("campaign_id, amount, currency, pledge_date").not("campaign_id", "is", null).neq("status", "בוטל").is("deleted_at", null),
    supabase.from("donations").select("campaign_id, amount, currency, donation_date").not("campaign_id", "is", null).is("deleted_at", null),
  ]);
