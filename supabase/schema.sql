...
-- =========================================================
-- 40. פרטי משפחה נוספים באיש קשר: ת"ז אישה, שם האב, שם החותן
--    *** קטע חדש בלבד ***
-- =========================================================
alter table contacts add column if not exists wife_id_number text; -- תעודת זהות אישה
alter table contacts add column if not exists father_name text; -- שם האב
alter table contacts add column if not exists father_in_law_name text; -- שם החותן

-- =========================================================
-- 41. זיכוי ידני ללקוח: מוסיפים ל-payment_method ערך "זיכוי" - זיכוי נרשם כרשומת
--    donations רגילה (payment_method='זיכוי', purpose='זיכוי') כך שהוא מקטין את
--    היתרה הפתוחה בדיוק כמו תשלום רגיל, אך מסומן ומוצג בנפרד בהיסטוריה/בפעילות
--    *** קטע חדש בלבד ***
-- =========================================================
...
alter table donations add constraint donations_payment_method_check
  check (payment_method in ('מזומן','צ''ק','כרטיס אשראי','העברה בנקאית','הוראת קבע','ביט','זיכוי'));

-- =========================================================
-- 42. הגדרות מיתוג הארגון (לוגו/שם/כותרת חתימה) - משמש למסמכים מעוצבים
--    (דו"ח מסכם, חשבונית) שנפתחים מכרטיס איש קשר. שורה יחידה גלובלית.
--    *** קטע חדש בלבד ***
-- =========================================================
create table if not exists org_settings (
  id boolean primary key default true check (id),
  org_name text not null default 'תרומיקס',
  logo_url text,
  report_header_note text, -- טקסט חופשי מתחת לשם הארגון בראש המסמך
  report_signature text, -- טקסט חתימה בתחתית המסמך (למשל שם החתום/תפקיד)
  updated_at timestamptz not null default now()
);
...
-- bucket ציבורי (לא רגיש - לוגו בלבד) כדי שתמונת הלוגו תיטען ישירות בדפי הדפסה/מייל
-- בלי צורך ב-signed URL
insert into storage.buckets (id, name, public)
values ('org-branding', 'org-branding', true)
on conflict (id) do nothing;

create policy "org_branding_storage_select" on storage.objects for select
  to authenticated using (bucket_id = 'org-branding');
create policy "org_branding_storage_insert" on storage.objects for insert
  to authenticated with check (bucket_id = 'org-branding' and my_role() = 'admin');
create policy "org_branding_storage_update" on storage.objects for update
  to authenticated using (bucket_id = 'org-branding' and my_role() = 'admin');
create policy "org_branding_storage_delete" on storage.objects for delete
  to authenticated using (bucket_id = 'org-branding' and my_role() = 'admin');

-- =========================================================
-- 43. תוכן גוף החשבונית (טקסט הנוסח המרכזי) וקישורי תשלום לפי מוקד תשלום -
--    להצגה כפתור/קישור בהודעת המייל שמלווה את המסמך
--    *** קטע חדש בלבד ***
-- =========================================================
alter table org_settings add column if not exists invoice_body_text text;
alter table org_settings add column if not exists payment_links jsonb;

-- =========================================================
-- 44. טקסט ברירת מחדל לגוף הודעת המייל בטופס השליחה (תומך {שם}/{כותרת} כתחליפים)
--    *** קטע חדש בלבד ***
-- =========================================================
alter table org_settings add column if not exists email_default_body text;

-- =========================================================
-- 45. כותרת ברירת מחדל נפרדת (במקום חישוב קבוע בקוד) להודעת המייל
--    *** קטע חדש בלבד ***
-- =========================================================
alter table org_settings add column if not exists email_default_subject text;

-- =========================================================
-- 46. יומן פעילות איש קשר: פעולות שאינן תרומה/התחייבות (הפקת/הדפסת/שליחת מסמך,
--    עדכון פרטי איש קשר וכו') - עם שיוך למי שביצע את הפעולה. תרומות/התחייבויות
--    עצמן ממשיכות להופיע דרך donations/pledges.created_by, לא כפולות כאן
--    *** קטע חדש בלבד ***
-- =========================================================
...
drop policy if exists "contact_activity_log_select" on contact_activity_log;
create policy "contact_activity_log_select" on contact_activity_log for select
  to authenticated using (true);
drop policy if exists "contact_activity_log_insert" on contact_activity_log;
create policy "contact_activity_log_insert" on contact_activity_log for insert
  to authenticated with check (true);

-- =========================================================
-- 47. משימות ותזכורות פר-איש-קשר: תזכורת עם תאריך יעד, שיוך למשתמש, השלמה,
--    "נודניק" (דחייה זמנית) ו"אל תציג שוב" (השתקת הפופ-אפ בלבד, לא מוחק את המשימה).
--    last_emailed_at מונע שליחת מייל כפולה על אותה תזכורת מכל הרצת cron
--    *** קטע חדש בלבד ***
-- =========================================================
...
-- =========================================================
-- 48. סנכרון אוטומטי של רשימת "מטפלים" (donation_handlers) עם משתמשי המערכת -
--    כל משתמש חדש (וגם משתמשים קיימים, מגיבוי חד-פעמי) נכנס לרשימת המטפלים לפי
--    שמו המלא, כדי שברירת המחדל "המשך טיפול ע"י המשתמש המבצע" תעבוד ושהתזכורות
--    האוטומטיות יוכלו לשייך משתמש אמיתי לפי שם המטפל שנבחר
--    *** קטע חדש בלבד ***
-- =========================================================
create or replace function sync_donation_handler_from_profile()
returns trigger as $$
begin
  if new.full_name is not null and length(trim(new.full_name)) > 0 then
    insert into donation_handlers (name)
    values (trim(new.full_name))
    on conflict (name) do nothing;
  end if;
  return new;
end;
...
drop trigger if exists on_profile_added_to_handlers on profiles;
create trigger on_profile_added_to_handlers
  after insert on profiles
  for each row execute function sync_donation_handler_from_profile();

insert into donation_handlers (name)
select trim(full_name) from profiles where full_name is not null and length(trim(full_name)) > 0
on conflict (name) do nothing;

-- =========================================================
-- 49. עמודת "פרטים" נפרדת ביומן הפעילות - כדי שעמודת "פעולה" תישאר תיאור קצר
--    (למשל "הורדת דו"ח מסכם") ועמודת "פרטים" תציג מידע נלווה בנפרד (קטגוריה,
--    סיבת זיכוי, סך יתרה, נמענים וכו') - עמודות מסודרות ולא טקסט אחד ארוך
--    *** קטע חדש בלבד ***
-- =========================================================
alter table contact_activity_log add column if not exists details text;

-- =========================================================
-- 50. צ'אט תמיכה: שיחה בין כל משתמש לבין הנהלת המערכת (admin). כל ההודעות של
--    משתמש נתון (משלו ומתשובות admin) משויכות ל-conversation_user_id אחד -
--    admin יכול לראות/לענות בכל השיחות, משתמש רגיל רואה/שולח רק בשיחה שלו עצמו.
--    support_thread_started_at על הפרופיל מאפשר "שיחה חדשה" מבחינת המשתמש (מסתיר
--    הודעות ישנות מהתצוגה שלו בלבד, בלי למחוק אותן - admin ממשיך לראות הכל)
--    *** קטע חדש בלבד ***
-- =========================================================
...
create index if not exists idx_support_messages_conversation on support_messages(conversation_user_id, created_at);
...
alter table profiles add column if not exists support_thread_started_at timestamptz;

-- =========================================================
-- 51. תיקון קריטי: יצירת משתמש חדש נכשלה עם "Database error creating new user"
--    כשנוצר דרך Supabase Dashboard / Admin API (לא דרך הרשמה עצמית באתר).
--    הסיבה: sync_donation_handler_from_profile() היא SECURITY DEFINER בלי search_path
--    מקובע, ומפנה ל-donation_handlers בלי ציון סכמה - כשהחיבור הפנימי שדרכו ה-Dashboard
--    יוצר משתמשים לא כולל public ב-search_path שלו, הטבלה לא נמצאת (שגיאת Postgres
--    42P01 "relation does not exist" אומתה בלוגים). מקבעים search_path במפורש
--    ומציינים public.donation_handlers כדי שזה יעבוד מכל נתיב יצירה
--    *** קטע חדש בלבד ***
-- =========================================================
create or replace function sync_donation_handler_from_profile()
returns trigger as $$
begin
  if new.full_name is not null and length(trim(new.full_name)) > 0 then
    insert into public.donation_handlers (name)
    values (trim(new.full_name))
    on conflict (name) do nothing;
  end if;
  return new;
...
-- =========================================================
-- 52. תיקון: מחיקת משתמש (Settings > Users > מחיקת חשבון) נכשלה עבור כל משתמש
--    שיש לו היסטוריית פעילות כלשהי - כמעט כל משתמש אמיתי. הסיבה: עמודות מעקב
--    כמו created_by/deleted_by/actor_id/assigned_to וכו' הצביעו על auth.users(id)
--    בלי on delete cascade/set null, אז Postgres חוסם את המחיקה (הפרת מפתח זר) ברגע
--    שהמשתמש יצר/ערך/מחק כל רשומה שהיא. מוסיפים on delete set null לכולן, כדי שמחיקת
--    משתמש תצליח ותשאיר את הרשומות ההיסטוריות עצמן (רק "מי ביצע" הופך ל-null)
--    *** קטע חדש בלבד ***
-- =========================================================
...
alter table contacts drop constraint if exists contacts_deleted_by_fkey,
  add constraint contacts_deleted_by_fkey foreign key (deleted_by) references auth.users(id) on delete set null;

alter table donations drop constraint if exists donations_created_by_fkey,
  add constraint donations_created_by_fkey foreign key (created_by) references auth.users(id) on delete set null;
alter table donations drop constraint if exists donations_deleted_by_fkey,
  add constraint donations_deleted_by_fkey foreign key (deleted_by) references auth.users(id) on delete set null;

alter table pledges drop constraint if exists pledges_created_by_fkey,
  add constraint pledges_created_by_fkey foreign key (created_by) references auth.users(id) on delete set null;
alter table pledges drop constraint if exists pledges_deleted_by_fkey,
  add constraint pledges_deleted_by_fkey foreign key (deleted_by) references auth.users(id) on delete set null;

alter table donation_categories drop constraint if exists donation_categories_deleted_by_fkey,
  add constraint donation_categories_deleted_by_fkey foreign key (deleted_by) references auth.users(id) on delete set null;

alter table donation_handlers drop constraint if exists donation_handlers_deleted_by_fkey,
  add constraint donation_handlers_deleted_by_fkey foreign key (deleted_by) references auth.users(id) on delete set null;

alter table contact_cities drop constraint if exists contact_cities_deleted_by_fkey,
  add constraint contact_cities_deleted_by_fkey foreign key (deleted_by) references auth.users(id) on delete set null;

alter table contact_files drop constraint if exists contact_files_uploaded_by_fkey,
  add constraint contact_files_uploaded_by_fkey foreign key (uploaded_by) references auth.users(id) on delete set null;
alter table contact_files drop constraint if exists contact_files_deleted_by_fkey,
  add constraint contact_files_deleted_by_fkey foreign key (deleted_by) references auth.users(id) on delete set null;

alter table contact_visibility_rules drop constraint if exists contact_visibility_rules_user_id_fkey,
  add constraint contact_visibility_rules_user_id_fkey foreign key (user_id) references auth.users(id) on delete set null;
alter table contact_visibility_rules drop constraint if exists contact_visibility_rules_created_by_fkey,
  add constraint contact_visibility_rules_created_by_fkey foreign key (created_by) references auth.users(id) on delete set null;
...
alter table campaigns drop constraint if exists campaigns_deleted_by_fkey,
  add constraint campaigns_deleted_by_fkey foreign key (deleted_by) references auth.users(id) on delete set null;

alter table campaign_call_logs drop constraint if exists campaign_call_logs_called_by_fkey,
  add constraint campaign_call_logs_called_by_fkey foreign key (called_by) references auth.users(id) on delete set null;

alter table campaign_reminders drop constraint if exists campaign_reminders_assigned_to_fkey,
  add constraint campaign_reminders_assigned_to_fkey foreign key (assigned_to) references auth.users(id) on delete set null;
alter table campaign_reminders drop constraint if exists campaign_reminders_created_by_fkey,
  add constraint campaign_reminders_created_by_fkey foreign key (created_by) references auth.users(id) on delete set null;

alter table campaign_contact_invitations drop constraint if exists campaign_contact_invitations_invited_by_fkey,
  add constraint campaign_contact_invitations_invited_by_fkey foreign key (invited_by) references auth.users(id) on delete set null;

alter table contact_activity_log drop constraint if exists contact_activity_log_actor_id_fkey,
  add constraint contact_activity_log_actor_id_fkey foreign key (actor_id) references auth.users(id) on delete set null;

alter table contact_tasks drop constraint if exists contact_tasks_assigned_to_fkey,
  add constraint contact_tasks_assigned_to_fkey foreign key (assigned_to) references auth.users(id) on delete set null;
alter table contact_tasks drop constraint if exists contact_tasks_created_by_fkey,
  add constraint contact_tasks_created_by_fkey foreign key (created_by) references auth.users(id) on delete set null;
...
-- support_messages.sender_id היה not null - הופך לניתן ל-null כדי ש-set null יעבוד
-- (ההודעה עצמה נשארת בהיסטוריית השיחה, רק שדה השולח מתאפס)
...
-- =========================================================
-- 53. קישור בין שורת שיק (donation_payment_lines) לבין קובץ הסריקה שהועלה בפועל
--    לתיקיית הקבצים של איש הקשר (contact_files) - כדי שכפתור "צפייה בשיק" בטופס
--    יעבוד גם בפתיחה מחדש של רשומה ישנה, לא רק בהעלאה באותה עריכה
--    *** קטע חדש בלבד ***
-- =========================================================
alter table donation_payment_lines add column if not exists file_storage_path text;

-- =========================================================
-- 54. עסקאות סליקת אשראי (נדרים פלוס) הממתינות לאישור שרתי - לפי תיעוד ה-API של נדרים
--    פלוס, אסור לסמוך על תגובת הצד-לקוח (postMessage) בלבד כדי לקבוע שתשלום בוצע,
--    כי היא ניתנת לזיוף. לכן: לפני פתיחת האייפרם נשמרים כאן כל פרטי הטופס (payload),
--    ורק לאחר שמתקבל אישור אמיתי מהשרת של נדרים פלוס (CallBack, מאומת לפי כתובת IP)
--    ב-webhook נוצרת בפועל רשומת ההתחייבות/תשלום - ראו app/api/nedarim-callback
--    *** קטע חדש בלבד ***
-- =========================================================
create table if not exists nedarim_pending_charges (
  id uuid primary key default gen_random_uuid(),
  created_at timestamptz not null default now(),
  confirmed_at timestamptz,
  status text not null default 'pending' check (status in ('pending', 'confirmed', 'failed')),
  flow text not null check (flow in ('payment_only', 'pledge_and_payment')),
  ...
  result_pledge_id uuid,
  result_surplus numeric(12, 2),
  result_surplus_currency text,
  error_message text
);

alter table nedarim_pending_charges enable row level security;

create policy "nedarim_pending_charges_select" on nedarim_pending_charges for select
  to authenticated using (true);

create policy "nedarim_pending_charges_insert" on nedarim_pending_charges for insert
  to authenticated with check (true);

-- =========================================================
-- 55. הודעת "המשתמש מחק את השיחה" בצ'אט תמיכה תסומן כהודעת מערכת (is_system) כדי
--    שתוצג בממשק הניהול ככיתוב קטן בין הודעות ולא כבועת שיחה שנשלחה, ומאפשר למנהל
--    למחוק שיחת תמיכה שלמה (מחיקה קשה - זהו יומן צ'אט, לא רשומה כספית/היסטורית)
--    *** קטע חדש בלבד ***
-- =========================================================
alter table support_messages add column if not exists is_system boolean not null default false;
