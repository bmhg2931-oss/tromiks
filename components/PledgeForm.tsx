    ...
    <form action={formAction} onChange={() => onDirty()} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="תאריך המשך טיפול *">
          <input type="date" name="follow_up" defaultValue={initial?.follow_up ?? ""} required className="in" />
        </Field>
        <Field label="המשך טיפול על ידי">
          <select name="handler" defaultValue={initial?.handler ?? currentUserName ?? ""} className="in">
