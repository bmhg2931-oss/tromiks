...
export default function ContactAutocomplete({
  ...
  async function handlePick(contact: Contact) {
    ...
    const [{ data: pledges }, { data: donations }] = await Promise.all([
      supabase.from("pledges").select("amount, currency").eq("contact_id", contact.id).neq("status", "בוטל").is("deleted_at", null),
      supabase.from("donations").select("amount, currency").eq("contact_id", contact.id).is("deleted_at", null),
    ]);
