...
import FollowUpConfirmModal from "./FollowUpConfirmModal";
import { createPledge, type PledgeFormResult } from "@/app/(app)/donations/pledge-actions";
...
export default function AddDonationModal({
  ...
  const [pendingFollowUp, setPendingFollowUp] = useState<PledgeFormResult | null>(null);
  ...
  function handlePledgeSaved(result?: PledgeFormResult) {
    if (result?.followUp && result.pledgeId && result.contactId) {
      setPendingFollowUp(result);
    } else {
      closeModal();
    }
  }
  ...
  return (
    <>
      ...
      {pendingFollowUp?.pledgeId && pendingFollowUp.contactId && (
        <FollowUpConfirmModal
          contactId={pendingFollowUp.contactId}
          pledgeId={pendingFollowUp.pledgeId}
          followUp={pendingFollowUp.followUp!}
          handler={pendingFollowUp.handler ?? null}
          category={pendingFollowUp.category ?? null}
          details={pendingFollowUp.details ?? null}
          handlers={handlers}
          onDone={() => {
