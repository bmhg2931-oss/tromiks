...
export type NedarimHandle = {
  // refId מזוהה כ-Param1 - נדרים פלוס מחזירים אותו חזרה בעדכון ה-CallBack, כדי שנוכל
  // לשייך את האישור השרתי לעסקה הממתינה הנכונה (ר' app/api/nedarim-callback)
  submitPayment: (mosad: string, apiValid: string, refId: string, fields: NedarimChargeFields) => void;
};
...
const NedarimIframe = forwardRef<NedarimHandle, { onResult: (result: NedarimTransactionResult) => void }>(
  function NedarimIframe({ onResult }, ref) {
    ...
    useImperativeHandle(ref, () => ({
      submitPayment(mosad, apiValid, refId, fields) {
        const callbackUrl =
          typeof window !== "undefined" ? `${window.location.origin}/api/nedarim-callback` : "";
        iframeRef.current?.contentWindow?.postMessage(
          {
            ...
            Value: {
              ...
              CallBackMailError: process.env.NEXT_PUBLIC_NEDARIM_CALLBACK_ERROR_EMAIL || "",
            },
          },
          "*"
        );
