...
export default function PaymentOnlyForm({
  ...
  async function handleCardSubmit() {
    ...
    const pending = await createPendingNedarimCharge(refId, "payment_only", fields, Number(amount), currency);
    if (!pending.ok) {
  ...
  function handleNedarimResult(result: NedarimTransactionResult) {
    ...
    // הדפדפן דיווח על הצלחה - אבל זו רק תגובת צד-לקוח שניתנת לזיוף. התשלום נחשב סופי
    // רק לאחר שמתקבל אישור אמיתי מהשרת של נדרים פלוס (CallBack), ולכן ממתינים לו כאן
    setCardSubmitting(false);
    setCardPolling(true);
  }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      if (result.status === "confirmed") {
        ...
        if (result.surplus && result.surplus > 0.5 && result.surplusCurrency && result.donationId) {
          setSurplusInfo({ donationId: result.donationId, amount: result.surplus, currency: result.surplusCurrency });
        } else {
          onSuccess();
        }
  ...
  return (
    <form action={formAction} onSubmit={handleFormSubmit} onChange={() => onDirty()} className="space-y-4">
          ...
          {!isNedarimSupportedCurrency(currency) ? (
            ...
            <>
              ...
              <button
                ...
                disabled={!amount || Number(amount) <= 0 || cardSubmitting || cardPolling}
      ...
      {paymentMethod && (
        <>
          <Field label="הערות">
            <textarea name="notes" value={notes} onChange={(e) => setNotes(e.target.value)} className="in min-h-[50px]" />
          </Field>
