...
import { lookupPostalCode } from "@/app/(app)/contacts/geocode-actions";
...
export default function ContactForm({
  ...
  const [country, setCountry] = useState(initial?.country ?? "ישראל");
  ...
  async function handleLookupPostalCode() {
    ...
    setPostalError(null);
    const res = await lookupPostalCode(streetRef.current?.value ?? "", houseNumberRef.current?.value ?? "", city, country);
    setPostalLoading(false);
    if (!res.ok) {
      setPostalError(res.error ?? "שגיאה בהשלמת מיקוד");
      return;
    }
  ...
  return (
    ...
    >
      ...
      <Section title="מגורים">
        <div className="grid grid-cols-2 gap-3">
          <Field label="רחוב">
            <input ref={streetRef} name="street" defaultValue={initial?.street ?? ""} disabled={readOnly} className="in" />
          </Field>
          <Field label="מספר">
            <input ref={houseNumberRef} name="house_number" defaultValue={initial?.house_number ?? ""} disabled={readOnly} className="in" />
          </Field>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <Field label="עיר">
          ...
          <div className="flex gap-2">
            ...
            {!readOnly && (
              <button
                ...
                className="text-xs border border-line rounded-lg px-3 whitespace-nowrap hover:bg-parchment disabled:opacity-50"
              >
                {postalLoading ? "מחפש..." : "השלמה אוטומטית"}
              </button>
            )}
          </div>
          {postalError && <p className="text-xs text-wine mt-1">{postalError}</p>}
        </Field>
      </Section>
      ...
      <Section title="פרטים נוספים">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="שם האב">
            <input name="father_name" defaultValue={initial?.father_name ?? ""} disabled={readOnly} className="in" />
          </Field>
          <Field label="שם החותן">
            <input name="father_in_law_name" defaultValue={initial?.father_in_law_name ?? ""} disabled={readOnly} className="in" />
          </Field>
        </div>
      ...
      <Section title="סיווג במערכת">
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="ת.ז. אישה">
            <input name="wife_id_number" defaultValue={initial?.wife_id_number ?? ""} disabled={readOnly} className="in" />
          </Field>
        </div>
        <div className="grid grid-cols-2 gap-3">
