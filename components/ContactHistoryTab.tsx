          ...
          <tbody>
            {rows.length > 0 ? (
              rows.map((r) => (
                <tr
                  key={r.id}
                  onClick={() => editable && r.paymentMethod !== "זיכוי" && setOpenRow(r)}
                  className={`border-b border-[#e6e3da] transition ${
                    editable && r.paymentMethod !== "זיכוי" ? "cursor-pointer hover:bg-parchment/50" : ""
                  }`}
                >
                  <td className="py-2.5 px-1.5">
                    <span className={`pill pill-${r.recordType === "payment" ? "paid" : "pending"} text-[11px]`}>
                      {r.recordType === "payment" && r.paymentMethod === "זיכוי" ? "זיכוי" : RECORD_TYPE_LABELS[r.recordType]}
                    </span>
                  </td>
